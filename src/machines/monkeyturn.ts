...
//
// ⚠️ 設定3の数値はどの出典サイトにも掲載が無い(なな徹・altema・DMMぱちタウンで確認済み。
//    1・2・4・5・6のみ公開)。実機は法規上6段階あるはずだが、解析サイトの慣習で設定3だけ
//    省いた5点表記になっているようだ。値を推測(補間)で埋めるのではなく、この機種の
//    settings 自体を5段階([1,2,4,5,6])にして「わからないものとして扱う」方針にしている。
// =============================================================================
...
const counterGroups = [
  ...
  },
  {
    ...
    categories: counterCategoriesFromPercentTable(settings, {
      ochitsuku: { label: '落ち着くんだ憲二…', percents: [50.0, 40.0, 40.0, 70.0, 40.0] },
      kehai: { label: 'この気配は！？', percents: [50.0, 60.0, 60.0, 30.0, 60.0] },
    }),
  },
...
const referencePoints = [
  {
    title: 'AT初当たり確率',
    body:
      'この機種の判別の土台。高設定ほどAT初当たりが軽くなる。設定3は出典未公開のため表から除外。' +
  ...
  },
  {
    ...
    body:
      ...
      '「落ち着くんだ憲二…」が設定5だけ突出して高い(70%)のが特徴。設定3は出典未公開のため表から除外。',
    table: {
      rows: [
        {
          label: '落ち着くんだ憲二…',
          valuesBySetting: { '1': '50%', '2': '40%', '4': '40%', '5': '70%', '6': '40%' },
        },
        {
          label: 'この気配は！？',
          valuesBySetting: { '1': '50%', '2': '60%', '4': '60%', '5': '30%', '6': '60%' },
        },
      ],
    },
  },
